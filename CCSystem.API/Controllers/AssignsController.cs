...
namespace CCSystem.API.Controllers
{
	...
	public class AssignsController : ControllerBase
	{
		...
		#region Get Assignments By Booking Detail
		/// <summary>
		/// Retrieves schedule assignments for a given booking detail.
		/// </summary>
		/// <param name="bookingDetailId">The ID of the booking detail.</param>
		/// <returns>A list of schedule assignments.</returns>
		/// <response code="200">Returns the list of assignments.</response>
		/// <response code="404">If the booking detail does not exist.</response>
		/// <response code="500">If there is an internal server error.</response>
		[ProducesResponseType(typeof(List<ScheduleAssignmentResponse>), StatusCodes.Status200OK)]
		[ProducesResponseType(typeof(Error), StatusCodes.Status404NotFound)]
		[ProducesResponseType(typeof(Error), StatusCodes.Status500InternalServerError)]
		[Produces(MediaTypeConstant.ApplicationJson)]
		[HttpGet(APIEndPointConstant.ScheduleAssign.GetScheduleAssignByBookingDetailIdEndpoint)]
		public async Task<IActionResult> GetAssignsByBookingDetail([FromRoute] BookingDetailIdRequest request)
		{
			ValidationResult validationResult = await _bookingDetailIdValidator.ValidateAsync(request);
			if (!validationResult.IsValid)
			{
				string errors = ErrorUtil.GetErrorsString(validationResult);
				throw new BadRequestException(errors);
			}
			var assignments = await _scheduleAssignService.GetAssignByBookingDetail(request.Id);
